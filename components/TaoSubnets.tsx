...
const TaoSubnets = () => {
    const subnets = [0, 6, 50, 65, 70, 71, 72, 73, 74, 75, 76, 77]
    const subnetIDs = subnets.join(',')
    const { data, error, isLoading } = useSWR(`/api/getSubnets?subnetIDs=${subnetIDs}`, fetcher)
                    ...
                    <tbody>
                        {
                            data && data.length > 1 && data.map((item: any, index: number) => (
                                <tr key={index}>
                                    ...
                                    <td className='text-center'>𝞃 {showTaoNumber(item.emission)}</td>
                                    <td className='text-center'>𝞃 {showTaoNumber(item.reg_cost)}</td>
                                </tr>
                            ))
                        }
                    </tbody>
